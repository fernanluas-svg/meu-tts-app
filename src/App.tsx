...
export default function App() {
  ...
  return (
    <div className="min-h-screen mesh-bg text-white selection:bg-violet-500/30 font-sans p-6 overflow-hidden flex items-center justify-center">
          ...
          <div className="glass-card p-6 flex flex-col h-full overflow-hidden">
            <header className="flex items-center gap-3 mb-8">
              ...
              <div>
                ...
                <p className="text-[10px] text-gray-500 font-mono uppercase tracking-widest mt-1">v1.1 PRO / Optimized</p>
              </div>
            </header>

            <div className="flex items-center justify-between mb-4">
              <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Catálogo</label>
              <div className="flex items-center gap-1.5">
                <div className="w-1.5 h-1.5 rounded-full bg-green-500" />
                <span className="text-[9px] font-mono text-gray-400">ONLINE</span>
              </div>
            </div>
                ...
                <div
                  ...
                  className={`w-full text-left p-4 rounded-2xl border transition-all relative group flex flex-col gap-2 ${
                    selectedVoiceId === voice.id 
                      ? 'voice-card-active' 
                      : 'glass-card hover:bg-white/5 border-transparent opacity-60 grayscale-[0.5] hover:grayscale-0'
                ...
                >
                  ...
                  >
                    <div className="flex justify-between items-start mb-1">
                      <span className={`font-semibold text-sm ${selectedVoiceId === voice.id ? 'text-white' : 'text-gray-200'}`}>
                        {voice.name}
                      </span>
                      {selectedVoiceId === voice.id && (
                    ...
                    <p className="text-[10px] text-gray-400 truncate mb-1">{voice.style}</p>
                    <p className="text-[9px] text-gray-500 line-clamp-1">{voice.description}</p>
                  </div>
            ...
            <div className="mt-8 space-y-6 pt-6 border-t border-white/5">
              <div className="space-y-4">
                <div className="flex justify-between mb-1">
                  <label className="text-[10px] text-gray-500 uppercase tracking-widest font-bold">Velocidade</label>
              ...
              <div className="space-y-4">
                <div className="flex justify-between mb-1">
                  <label className="text-[10px] text-gray-500 uppercase tracking-widest font-bold">Tom (Pitch)</label>
        ...
        <div className="flex-1 flex flex-col gap-6 h-full min-w-0">
            ...
            <section className="glass-card p-8 flex flex-col h-full relative overflow-hidden group">
              <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-gray-400">Roteiro Principal</span>
                </div>
                <div className="flex gap-4">
                  <button 
                    onClick={() => setText('')}
                    disabled={isProcessing || !text}
                    className="text-xs text-gray-500 hover:text-white transition-colors disabled:opacity-30 flex items-center gap-1.5"
                  >
                    <Trash2 size={12} />
                    Limpar
                  </button>
                  <span className="text-xs px-2 py-1 bg-white/5 rounded text-gray-500 font-mono">
              ...
              <textarea 
                ...
                disabled={isProcessing}
                className="flex-1 bg-transparent border-none outline-none resize-none text-xl md:text-2xl leading-relaxed text-gray-200 placeholder-white/5 font-light custom-scrollbar"
              ...
              <div className="mt-8 flex gap-4 shrink-0">
                {segments.length > 0 && !isProcessing && (
                  <button 
                    onClick={resetAll}
                    className="h-16 px-8 rounded-2xl glass-card font-bold text-gray-400 hover:text-white transition-all flex items-center justify-center gap-3 border border-white/5"
                  >
                    <RotateCcw size={20} />
                    RESET
                  </button>
          ...
          <AnimatePresence>
            {(showMonitor || segments.length > 0) && (
              ...
              >
                <div className="p-4 border-b border-white/5 bg-white/2 flex items-center justify-between sticky top-0 z-10">
                  <div className="flex items-center gap-4">
                    <div className="flex flex-col">
                      <div className="flex items-center gap-2">
                        <Activity size={16} className={isProcessing ? "text-violet-400" : "text-gray-500"} />
                  ...
                  <div className="flex items-center gap-8">
                    <div className="text-right">
                      <p className="text-[9px] text-gray-500 font-bold uppercase">Queue</p>
                      <p className="font-mono text-[10px] font-bold text-violet-400">
                        {completedSegments} / {segments.length} BLOCKS
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="text-[9px] text-gray-500 font-bold uppercase">Progress</p>
                    ...
                    <button 
                      ...
                      className="text-gray-500 hover:text-white px-2 py-1 rounded hover:bg-white/5 transition-colors"
                    >
                ...
                {showMonitor && (
                  <div className="grid grid-cols-[1fr_250px] flex-1 overflow-hidden">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                        {segments.map((seg, idx) => (
                          ...
                          >
                            ...
                            <p className="line-clamp-1 text-gray-300 italic opacity-80">{seg.text}</p>
                          </div>
                        ))}
                      </div>
                          ...
                          <div className="flex items-center justify-between gap-4">
                            <div className="flex items-center gap-4 flex-1 min-w-0">
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="text-[9px] font-bold text-white mb-1 uppercase">MASTER FILE READY</p>
                                <audio src={finalAudioUrl} controls className="w-full h-8 filter invert hue-rotate-180 brightness-200" />
                              </div>
                            </div>
                            <button 
                    ...
                    <div className="border-l border-white/5 bg-black/40 flex flex-col">
                      <div className="p-2 border-b border-white/5 bg-white/2 flex items-center gap-2 text-[9px] font-bold text-gray-500 uppercase">
                        <Database size={10} />
                        Sync Logs
                      </div>
                      <div className="flex-1 overflow-y-auto p-3 font-mono text-[9px] space-y-1.5 flex flex-col-reverse custom-scrollbar">
                        {logs.map((log, i) => (
                          <div key={i} className="text-gray-500 border-l border-white/10 pl-2 py-0.5">
                            {log}
                          </div>
                        ))}
                      </div>
